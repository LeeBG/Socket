...
function view(seq, cud_cd) {
	...
	form.action = "<c:url value="/policy/destObjectPolicy/destObjectGroupView.lin" />";
	form.submit();
}
...
function checkDelete() {
	...
	if (confirm("선택 목록을 삭제하겠습니까?")) {
		var requestURL = "<c:url value="/policy/destObjectPolicy/deleteDestObjectGroup.lin" />";
								...
								<c:if test="${not empty destObjectGroupList}">
									<c:forEach items="${destObjectGroupList}" var="destObjectGroupList" varStatus="c">
										...
										<tr>
											...
											<td class="Rborder t_center ${textColorClass}" title="<c:out value="${destObjectGroupList.obj_group_seq}" />" onclick="view('${destObjectGroupList.seq}', '${CUD_CD_U}')">
												<c:out value="${destObjectGroupList.obj_group_seq}" />
											</td>
											<td class="Rborder t_center ${textColorClass}" title="<c:out value="${destObjectGroupList.obj_group_nm}" />" onclick="view('${destObjectGroupList.seq}', '${CUD_CD_U}')">
												<c:out value="${destObjectGroupList.obj_group_nm}" />
											</td>
											<td class="Rborder t_center ${textColorClass}" title="<c:out value="${destObjectGroupList.obj_seq_info}" />" onclick="view('${destObjectGroupList.seq}', '${CUD_CD_U}')">
												<c:out value="${destObjectGroupList.obj_seq_info}" />
											</td>
											<td class="Rborder t_center ${textColorClass}" title="<c:out value="${destObjectGroupList.note}" />" onclick="view('${destObjectGroupList.seq}', '${CUD_CD_U}')">
												<c:out value="${destObjectGroupList.note}" />
											</td>
											<td class="Rborder t_center ${textColorClass}" 
												title="<c:choose><c:when test="${destObjectGroupList.del_yn == 'N'}">사용</c:when><c:when test="${destObjectGroupList.del_yn == 'Y'}">미사용</c:when></c:choose>" onclick="view('${destObjectGroupList.seq}', '${CUD_CD_U}')">
												<c:choose>
													<c:when test="${destObjectGroupList.del_yn == 'N'}">사용</c:when>
													<c:when test="${destObjectGroupList.del_yn == 'Y'}">미사용</c:when>
												</c:choose>
											</td>
											<td class="Rborder t_center ${textColorClass}" title="<c:out value="${destObjectGroupList.crt_id}" />" onclick="view('${destObjectGroupList.seq}', '${CUD_CD_U}')">
											...
											<td class="Rborder t_center ${textColorClass}" title="<fmt:formatDate value="${destObjectGroupList.crt_date}" pattern="yyyy-MM-dd HH:mm" />" 
															onclick="view('${destObjectGroupList.seq}', '${CUD_CD_U}')">												
