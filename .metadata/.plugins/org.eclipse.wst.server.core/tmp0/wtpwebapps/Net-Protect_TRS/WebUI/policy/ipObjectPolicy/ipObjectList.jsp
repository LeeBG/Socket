...
function checkDelete() {
	...
	if (confirm("선택 목록을 삭제하겠습니까?")) {
		var requestURL = "<c:url value="/policy/ipObjectPolicy/deleteIpObject.lin" />";
			...
			<div class="conWrap">
				...
				<div class="conBox">
					<div class="topCon nBorder"  >
						...
						<caption>객체ID, 객체명, 객체 시작IP, 객체 종료IP, 객체구분, 삭제여부, 생성일, 생성자, 수정일, 수정자</caption>
							<colgroup>
								<col style="width:10%;"/>
								<col style="width:20%;" />
								<col style="width:10%;" />
							...
							<tbody>
								<c:if test="${not empty ipObjectList}">
									<c:forEach items="${ipObjectList}" var="viewipObjectForm" varStatus="c">
										...
										<tr>
											<c:choose>
												<c:when test="${viewipObjectForm.isdel_yn eq 'N'}">
											...
											<td class="Rborder t_center ${textColorClass}" title="<c:out value="${viewipObjectForm.obj_seq}" />" onclick="view('${viewipObjectForm.seq}', '${CUD_CD_U}')">
												<c:out value="${viewipObjectForm.obj_seq}" />
											</td>
											<td class="Rborder t_center ${textColorClass}" title="<c:out value="${viewipObjectForm.obj_nm}" />" onclick="view('${viewipObjectForm.seq}', '${CUD_CD_U}')">
												<c:out value="${viewipObjectForm.obj_nm}" />
											</td>
											<td class="Rborder t_center ${textColorClass}" title="<c:out value="${viewipObjectForm.src_st_ip}" />" onclick="view('${viewipObjectForm.seq}', '${CUD_CD_U}')">
												<c:choose>
													<c:when test="${viewipObjectForm.obj_cd eq '1'}">-</c:when>
													<c:otherwise><c:out value="${viewipObjectForm.src_st_ip}" /></c:otherwise>
												</c:choose>
											</td>
											<td class="Rborder t_center ${textColorClass}" title="<c:out value="${viewipObjectForm.src_ed_ip}" />" onclick="view('${viewipObjectForm.seq}', '${CUD_CD_U}')">
												<c:choose>
													<c:when test="${viewipObjectForm.obj_cd eq '1'}">-</c:when>
											...
											<td class="Rborder t_center ${textColorClass}" title="<c:out value="${viewipObjectForm.net_mask}" />" onclick="view('${viewipObjectForm.seq}', '${CUD_CD_U}')">
												<c:choose>
													<c:when test="${viewipObjectForm.obj_cd eq '1'}">-</c:when>
													<c:when test="${viewipObjectForm.obj_cd eq '2'}">-</c:when>
											...
											<td class="Rborder t_center ${textColorClass}" title="<c:out value="${viewipObjectForm.max_connection}" />" onclick="view('${viewipObjectForm.seq}', '${CUD_CD_U}')">
											...
											<td class="Rborder t_center ${textColorClass}" 
												...
												onclick="view('${viewipObjectForm.seq}', '${CUD_CD_U}')">
												<c:choose>
											...
											<td class="Rborder t_center ${textColorClass}" 
												title="<c:choose><c:when test="${viewipObjectForm.del_yn eq 'Y'}">미사용</c:when><c:when test="${viewipObjectForm.del_yn eq 'N'}">사용</c:when></c:choose>" 
													onclick="view('${viewipObjectForm.seq}', '${CUD_CD_U}')">
												<c:choose>
											...
											<td class="Rborder t_center ${textColorClass}" title="<c:out value="${viewipObjectForm.crt_id}" />" onclick="view('${viewipObjectForm.seq}', '${CUD_CD_U}')">
												<c:out value="${viewipObjectForm.crt_id}" />
											</td>
											<%-- <td class="Rborder t_center ${textColorClass}" title="<c:out value="${viewipObjectForm.mod_date}" />" onclick="view('${viewipObjectForm.seq}', '${CUD_CD_U}')">
												<c:out value="${viewipObjectForm.mod_date}" />
											</td>
											<td class="Rborder t_center ${textColorClass}" title="<c:out value="${viewipObjectForm.mod_id}"/>" onclick="view('${viewipObjectForm.seq}', '${CUD_CD_U}')">
												<c:out value="${viewipObjectForm.mod_id}"/>
											</td> --%>
										</tr>
									</c:forEach>
								</c:if>  
