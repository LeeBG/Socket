...
<script type="text/javascript">
	...
	function updateFMPL0010(){
		...
		if (confirm("저장하지 않은 변경 값이 있습니다. 변경된 값을 저장하시겠습니까?")) {
			if(programArray != null || programArray != ""){
				var requestURL = "<c:url value="/policy/authPolicy/updateAuthPolicyMgt.lin" />";
				var successURL = "<c:url value="/policy/authPolicy/authPolicyMgt.lin" />";
				resultCheckFunc($("#lform"), requestURL, function(response) {
	...
	function openPopup(cud_cd) {
		...
		var auth_cd = $(':radio[name="authInfo"]:checked').val();
		var url = "<c:url value="/policy/authPolicy/authPolicyMgtPopup.lin" />?cud_cd="+cud_cd+"&auth_cd="+auth_cd;
	...
	function deleteAuth(cud_cd){
		...
		if (auth_cd == null) {
			...
			if (confirm(" 정말로 삭제 하시겠습니까?")) {
				...
				var requestURL = "<c:url value="/policy/authPolicy/insertAuthPolicyMgt.lin" />";
				var successURL = "<c:url value="/policy/authPolicy/authPolicyMgt.lin" />";
...
<body>
<form id="lform" name="lform" onsubmit="return false;" method="post" action="<c:url value="/policy/authPolicy/authPolicyMgt.lin" />">
...
<input id="type" name="type" type="hidden" value = "T"/>
...
<div class="rightArea">
	<div class="conWrap trisectionWrap">
							...
							<c:forEach items="${authInfoList}" var="authInfo">
									<tr>
										<td class="Rborder">
											...
											<label style="cursor:pointer"><input type="radio" onchange="isDelYn('${authInfo.isdel_yn}')" class="input_radio" name="authInfo" value="${authInfo.auth_cd}"/>${authInfo.auth_cd_nm}</label>
										</td>
									</tr>
							</c:forEach>
						</tbody>
		...
		<div class="conWrap trisection right">
			...
			<div class="conBox">
				<div class="tab_box">
					<div id ="tab" class="Rborder tab on" onclick="selectFMPL0020('T')">
