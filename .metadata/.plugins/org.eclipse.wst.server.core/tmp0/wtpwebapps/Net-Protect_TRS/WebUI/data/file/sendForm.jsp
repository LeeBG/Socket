...
var isValidApproval = '${isValidApproval}';
	...
	$(document).ready(function() {
		...
		<c:if test="${getSiteCode eq 'komsco'}">
			app_manager.initLastApproverNoRelationDept();
		</c:if>
		noticeApprovalLockToApprover("Y", afterAppLockPeriod, "${networkPosition}");

		blockImgDragEvent();
		validCheckComment();
		setSiteLogic("${getSiteCode}");
	});

	function setSiteLogic(siteCode) {
		if(siteCode == "mcst" || siteCode == "museum" || siteCode == "nl") {
			$(".sendform textarea[name='comment']").css("height", "150px");
			$(".fileUpLoadBox.middle").css("min-height", "180px");
			$(".fileUpLoadBox.middle").css("max-height", "185px");
			$("#file_uploader").css("height", "150px");
	...
	function fileupload() {
		...
		function isCanUploadFile(){
			...
			<c:if test="${isApprovalUse}">
				...
				selfAppInfoDisplay : 자가결재시 결재자 정보 표시 여부
				--%>
				if(('${selfAppInfoDisplay}' == 'N') && ($("#selfAppYn").val() == 'Y') && (app_manager.validateSelfApprovalChecked('<spring:message code="data.file.sendForm.script.selfApproval.request.confirm" />', '<spring:message code="data.file.sendForm.script.selfApproval.request.valid" />') == false) ) 
					return;
				...
				<%-- gzip 파일 형식 검사 --%>
				if( app_manager.checkGzipFile() ) {
					alert("<spring:message code="data.file.sendForm.script.invalid.gzip" />");
					return false;
				}
...
<script type="text/javascript">
	...
	function initTransfer() {
		...
		if(siteCode == 'mma'){
			app_manager.initApprovalSettingCustom();
		}else{
			app_manager.initApprovalSetting("${AfApprovalCheckYn}");
		}	
...
function validCheckComment(){
	$("#select_comment").on('change', function(){
		$("#comment").val($("#select_comment option:selected").val());
		if($("#select_comment").val() == $("#select_comment option:first").val()){
			$('#comment').val('');
		}
		chgTextareaWdLength($('#comment')[0],500,"${customfunc:getMessage('data.file.sendForm.content.comment')}");
	});
}
...
<div class="topWarp">
	<div class="titleBox">
		<h2 class="f_left text_bold">자료전송</h2>
...
<div class="conWrap sendform">
	<h3>
		...
		<c:choose>
			<c:when test="${ getSiteCode eq 'mcst' || getSiteCode eq 'nl' }">
				<spring:message code="data.left.data.send.${ networkPosition}" /> &nbsp; <spring:message code="data.file.sendForm.preservation.info.txt" arguments="${rcvKeepTerm}" />
			</c:when>
			<c:otherwise>
		...
		<div class="optionArea">
			...
			<div class="optionArea_rightBox f_right">
				<div class="approvalInfo">
					...
					<p id="proxy_approvalBox" class="lBar mg_l20 mg_t4" <c:if test="${getSiteCode eq 'mcst' || getSiteCode eq 'museum' || getSiteCode eq 'sejong_nl' || getSiteCode eq 'nl'}">style="display:none;"</c:if>>
				...
				<thead>
					<!-- <tr>
						<td>
							...
							<input type="text" class="text_input max mg_l30" name="title" id="title" value="" onkeyup="onlySizeFillter(this,100)"/>
						</td>
					</tr> -->
					<tr>
						<td>
							...
							<c:if test="${getSiteCode ne 'kbcard'}">
								※ 미입력시 파일명으로 자동입력 전송됩니다. 
							</c:if>
							(입력시 ${(titleSizeMin ne '' and titleSizeMin ne null and titleSizeMin ne 0) ? titleSizeMin:5}자 이상 ~ ${(titleSizeMax ne '' and titleSizeMax ne null and titleSizeMax ne 0) ? titleSizeMax:50}자 이하로 입력하세요.)
							</c:if>
							...
							<c:if test="${comment_template_yn}">
								<div id="comment-template"><label for="comment_template">${customfunc:getMessage('data.file.sendForm.content.comment_template')}</label>
									<select id="select_comment" style="margin-left:16px;">
							...
							<textarea class="text_input max mg_l30 item-align-middel korean_ime_mode pd_l1" name="comment" id="comment" oninput="chgTextareaWdLength(this,${commentSizeMax},'${customfunc:getMessage('data.file.sendForm.content.comment')}')"></textarea>
							<div id="comment-remain-word" class="mg_l5 mg_b5"><span id="comment-use-legnth">0</span>/<span>${commentSizeMax}</span> 자</div>
							<c:if test="${getSiteCode eq 'kins'}">
							<div class="upload_btn_area" style="padding-top: 20px;">
								<div class="left_btn" style="float:left;">
									<button type="button" class="btn_common theme" onclick="clickInputFile( file_manager, '${upCnt}')"><img class="btn_icon" alt="파일추가아이콘" src="/Images/icon/icon_file_01.png"/><span>파일추가</span></button>
								</div>
								<div class="right_btn" style="float: left; padding-left: 10px;">
									<button type="button" class="btn_common theme" onclick="filedelete()"><img class="btn_icon" alt="파일삭제아이콘" src="/Images/icon/icon_file_02.png"/><span>파일삭제</span></button>
								</div>
							</div>
							</c:if>
						</td>
					...
					<tr <c:if test="${getSiteCode eq 'mcst' || getSiteCode eq 'museum' || getSiteCode eq 'motie' || getSiteCode eq 'sejong_nl'}">class="none"</c:if>>
...
<!-- 		<div id="progress" align="center">0% complete</div>
		...
		<div class="btn_area_left mg_t10 mg_b10 mg_l10 file_web">
			<table class="button_table">
				<tr>
					<c:if test="${getSiteCode ne 'kins'}">
					<td class="h_25 w_112">
						<button type="button" class="btn_common theme" onclick="clickInputFile( file_manager, '${upCnt}')"><img class="btn_icon" alt="파일추가아이콘" src="/Images/icon/icon_file_01.png"/><span>파일추가</span></button>
					</td>
					</c:if>
					<td rowspan="3" class="h_50 w_112">
						<button type="button" class="btn_common theme" id="send_button" onclick="fileupload('web');"><span class="text"></span><img class="loading_img" alt="자료전송로딩바" src="/Images/fileUpload/loading.gif"></button>
					</td>
					<td rowspan="3" class="notice">
						<div id="restrictNotice"></div>
						<c:forEach var="notice" items="${sendNotice}">
							<p>${notice}</p>
						</c:forEach>
