...
<c:set var="isOuterExtApprovalUse" value="${sessionScope.loginUser.outerFilePolicy.exts_ap_yn}" />
<c:set var="isInnerExtApprovalUse" value="${sessionScope.loginUser.innerFilePolicy.exts_ap_yn}" />
<c:set var="loginUser" value="${sessionScope.loginUser}" />
<c:choose>
<c:when test="${isApprovalUse || (isOuterExtApprovalUse eq 'Y' && !isApprovalUse) || (isInnerExtApprovalUse eq 'Y' && !isApprovalUse)}">
...
<div class="topWarp">
	<div class="titleBox">
		<h2 class="f_left text_bold">보낸 자료</h2>
		<p class="breadCrumbs">자료송수신 > 보낸 자료</p>
	</div>
</div>
<div class="conWrap">
	<h3>검색조건</h3>
				...
				<tbody>
					<tr>
						<td class="t_center">
							...
							<input id="searchValue" name="searchValue" type="text" class="text_input long" style="max-width:70%;" value="${dataForm.searchValue}" placeholder="검색어를 입력해주세요." onkeyup="onlySizeFillter(this,100);" onkeydown="return keyDownEnterDoSearch();"/>
		...
		<div class="table_area_style01 hoverTable">
			...
			<caption>삭제유무, 중요, 제목, 파일, 전송일</caption>
				<colgroup>
					...
					<col style="width:13%;" />
					<col style="width:13%;" />
					<c:if test="${isApprovalUse || (isOuterExtApprovalUse eq 'Y' && !isApprovalUse) || (isInnerExtApprovalUse eq 'Y' && !isApprovalUse)}">
						<col style="width:8%;" />
					</c:if>
				</colgroup>
				<thead>
						...
						<c:forEach items="${dataList}" var="data">
							...
							<tr id="tr_id_${data.data_seq}">
								...
								<td onClick="read('${data.data_seq}'); return false;">
									...
									<c:out value="${data.title }" escapeXml="true" /><!-- </a> --></td>
