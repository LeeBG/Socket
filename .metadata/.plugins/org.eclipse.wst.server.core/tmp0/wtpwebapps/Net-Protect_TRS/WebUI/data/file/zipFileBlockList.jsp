			...
			<div class="conBox">
				<div class="popCon">
					<div class="table_area_style01 mg_t20 mg_b10">
						<p style="color: #e05050; margin-bottom:5px;">※ 마임타입 검사중에 검출된 비정상 파일 목록입니다.</p>
						<table id="checkMimeTypeTable" summary="비정상 파일 리스트" style="table-layout : fixed; border:1px solid #ddd; ">
							...
							<tbody>
								<c:forEach items="${fn:split(msg, '|') }" var="item">
									...
									<c:choose>
										...
										<c:when test="${fn:contains(item, '중첩압축')}">
											<c:set var="status">vc_scanStatusText n12</c:set>
										</c:when>
									<c:otherwise>
									...
									<c:forEach items="${fn:split(item, ',') }" var="list"  varStatus="cnt">
										<td class="Rborder ${cnt.count ne 1 ? 't_center' : '' } ${status}">${list}</td>
									</c:forEach>
									</tr>
								</c:forEach>
							</tbody>
