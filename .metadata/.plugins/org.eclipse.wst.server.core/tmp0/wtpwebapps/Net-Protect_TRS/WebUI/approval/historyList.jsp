...
<script type="text/javascript">
	...
	function read(approval_seq, data_seq, np_cd) {
		var url = "<c:url value="/approval/approvalView.lin" />?approval_seq=" + approval_seq + "&data_seq=" + data_seq+"&view_type=Y" + "&np_cd=" + np_cd;
...
<body>
<form id="lform" name="lform" onsubmit="return false;" method="post" action="<c:url value="/approval/historyList.lin" />">
...
<div class="topWarp">
	<div class="titleBox">
		<h2 class="f_left text_bold">결재 이력함</h2>
		<p class="breadCrumbs">결재관리 > 결재 이력함</p>
	</div>
</div>
	...
	<div class="conBox">
		<div class="table_area_style01 hoverTable">
			...
			<caption><spring:message code="common.approval.status"/>,결재요청자,결재제목,결재요청시각,결재시각</caption>
				...
				<tbody>
				<c:choose>
					<c:when test="${oppsiteApprovalOption ne 'N'}">
				...
				<c:forEach items="${approvalList}" var="approval">
					<tr id="tr_id_${approval.approval_seq}" onClick="read('${approval.approval_seq}','${approval.data_seq}','${approval.np_cd}'); return false;">
						...
						<c:if test="${oppsiteApprovalOption ne 'N'}">
							<td class="right_line t_center"><span title="${approval.np_cd}">${customfunc:codeDes("NP_CD", approval.np_cd) }</span></td>
						</c:if>
						...
						<td class="right_line"><c:out value="${approval.title}" /></td>
