...
<script type="text/javascript">
	...
	function search(){
		...
		frm.action="<c:url value="/fm/lg/FMLG1020.lin" />";
		frm.submit();
		showModal();
	}
	...
	function view(data_seq , searchPosition, ord){
		...
		var url = "<c:url value="/fm/lg/FMLG0010.lin" />?data_seq=" + data_seq + "&networkPosition=" + networkPosition + "&view=apt";
	...
	function download(){
		...
		frm.action = '<c:url value="/fm/lg/aptFileListExcelDownload.lin" />';
		frm.submit();
		frm.action="<c:url value="/fm/lg/FMLG1020.lin" />";
	}
...
<div class="rightArea">
	<div class="topWarp">
		<div class="titleBox">
			<h2 class="f_left text_bold">APT 검출 이력</h2>
			<p class="breadCrumbs">로그관리 > APT검출이력</p>
		</div>
	</div>
	<div class="conWrap">
		<h3>검색조건</h3>
		...
		<div class="conBox">
			<div class="table_area_style01 hoverTable">
				...
				<caption>파일명,전송위치,탐지명,사용자,전송시간</caption>
					...
					<tbody>
					<c:choose>
					<c:when test="${not empty aptAttachFileFormList}">
						<c:forEach items="${aptAttachFileFormList}" var="AptAttachFileForm">
						<tr onclick="view('${AptAttachFileForm.data_seq}','${AptAttachFileForm.np_cd}', '${AptAttachFileForm.ath_ord}')">
							<c:if test="${AptAttachFileForm.np_cd eq 'I'}"><td class="t_center Rborder">${INNER} -> ${OUTER}</td></c:if>
							<c:if test="${AptAttachFileForm.np_cd eq 'O'}"><td class="t_center Rborder">${OUTER} -> ${INNER}</td></c:if>
							<td class="t_center Rborder"><c:out value="${AptAttachFileForm.users_nm}(${AptAttachFileForm.crtr_id})"/></td>
							<td class="t_center Rborder"><c:out value="${AptAttachFileForm.dept_nm}"/></td>
							<td class="t_center Rborder"><spring:message code="apt.status.block.message" /></td>
							<td class="t_center Rborder"><c:out value="${AptAttachFileForm.file_nm}"/></td>
							<td class="t_center Rborder"><c:out value="${customfunc:BigFileSizeFormat(AptAttachFileForm.file_size)}"/></td>
							<td class="t_center Rborder"><c:out value="${AptAttachFileForm.connect_ip}"/></td>
							<td class="t_center Rborder" >
								<fmt:formatDate value="${AptAttachFileForm.crt_time}" pattern="yyyy-MM-dd HH:mm" />
							</td>
						</tr>
						</c:forEach>
					</c:when>
