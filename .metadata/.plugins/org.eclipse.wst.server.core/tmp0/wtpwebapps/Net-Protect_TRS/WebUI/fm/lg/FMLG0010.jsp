...
<script type="text/javascript">
	...
	function view(idx){
		...
		var url = "<c:url value="/fm/lg/FMLG0011.lin" />";
	...
	function dlpview(ath_ord){
		...
		var frm = document.popform;
		var url = "<c:url value="/fm/lg/FMLG0012.lin" />";
	...
	function resetDownloadCnt( fileid ){
		if( confirm('다운로드 횟수를 초기화하시겠습니까?') ){
			if( fileid == 'all' ){
				$("#ath_ord").val( 0 );
			}else{
				$("#ath_ord").val( fileid );
			}
			var requestURL = '<c:url value="/data/file/resetDownloadCnt.lin" />';
			resultCheckFunc($("#lform"), requestURL, function(response) {
				if( response && response['result'] == 'F' ){
					alert('다운로드 횟수 초기화에 실패하였습니다.')
				}else{
					location.reload();
				}
			});
...
<body>
<form id="extform" method="post" action="" onSubmit="return false;">
<input type="hidden" name="csrf" value="${loginUser.csrf }" />
...
<div class="wrap" id="fmlg0010">
	...
	<div class="conBox">
		...
		<div class="data_detail_box">
			...
			<dl>
				<dt>사용자 : </dt>
				<dd>${requestUser.users_nm}(${requestUser.users_id})</dd>
			</dl>
			<%-- <dl>
				<dt>수신인 : </dt>
			...
			<c:choose>
				<c:when test="${data.attachFileType eq 'normal'}">
					<div class="text_color_r mg_t15 lh_20">※ 요청 건의 파일 중 정상 파일만 표시합니다.<br>※ 전송 파일 ${data.ath_file_cnt}개 중 ${data.attachFileFormList.size()}개가 정상 파일입니다.</div>
				</c:when>
				<c:when test="${data.attachFileType eq 'notnormal'}">
					<div class="text_color_r mg_t15 lh_20">※ 요청 건의 파일 중 비정상 파일만 표시합니다.<br>※ 전송 파일 ${data.ath_file_cnt}개 중 ${data.attachFileFormList.size()}개가 비정상 파일로 검출되었습니다.</div>
				</c:when>
			</c:choose>
			<c:if test="${param.view eq 'dlp'}">
				<div class="text_color_r mg_t15 lh_20">※ 전송 파일 ${data.attachFileFormList.size()}개 중 ${data.dlp_file_cnt }개가 개인정보검출 파일입니다.</div>
			</c:if>
			<c:if test="${param.view eq 'apt'}">
				<div class="text_color_r mg_t15 lh_20">※ 전송 파일 ${data.attachFileFormList.size()}개 중 ${data.apt_file_cnt }개가 APT 검출 파일입니다.</div>
			</c:if>
			<dl class="last_item div_fmlg001">
				<dt class="last_item blind">첨부파일</dt>
				<dd class="last_item">
					<div class="d_table" style="width:100%;">
						<c:if test="${data_networkPosition eq 'O' && data.status ne 'SS' }">
							<div class="d_tablecell va_bottom text_color_r" style="width:60%;">※ ${OUTER}의 전송완료되지 않은 파일은 다운로드 불가합니다.</div>
						</c:if>
						<c:if test="${downloadMaxCnt ne null && param.view ne 'notnormal'}">
							<div class="d_tablecell t_right" style="width:40%;">
								<button class="btn_common theme" onclick="resetDownloadCnt('all'); this.onclick=null;">전체 다운로드 횟수 초기화</button>
							</div>
						</c:if>
					</div>
					<table class="thBox mg_t10 layout_fixed" style="overflow:visible;">
						<tr>
							<th class="allChk c_check"><input ${admin_download_yn eq 'Y' ? '' : 'disabled'} type="checkbox" id="allChk" name="allChk" class="input_chk" onclick="togchk(this, 'chk');"/></th>
							...
							<c:if test="${downloadMaxCnt ne null}">
								<th class="fileDownlaod c_download border_l" style="line-height:normal;">
									<spring:message code="receive.file" /><br>
									<spring:message code="data.popup.filedownload" />
								</th>
							</c:if>
							<c:if test="${user_download_yn eq 'Y'}"><th class="c_download_time border_l"><spring:message code="data.popup.filedownload_time" /></th></c:if>
						</tr>
					</table>
					<div class="scrollBox">
						<table class="layout_fixed" cellspacing="0" cellpadding="0" border="0" summary="자료전송 정보를 상세히 볼 수 있는 표입니다.">
							...
							<c:if test="${fn:length(data.attachFileFormList) > 0}">
								...
								<c:set var="isCompressFile" value="${customfunc:isCompressFile(attachFile.file_ext)}"/>
									...
									<c:choose>
									<c:when test="${attachFile.vc_status eq 3 || attachFile.vc_status eq 4}">
										<c:set var="delYn" value="Y"/>
									</c:when>
									<c:otherwise>
										<c:set var="delYn" value="${attachFile.in_del_yn }"/>
										<c:if test="${(loginUser.auth_cd ne 4 && delYn eq 'N') || (param.view eq 'notnormal' && delYn eq 'N')}">
									...
									<c:when test="${delYn eq 'N'}">
										...
										<c:if test="${(1 != attachFile.vc_status && 6 != attachFile.vc_status && 13 != attachFile.vc_status)}">
											...
											<c:set var="fileVolume"><spring:message code="data.dataView.data.attachFile.scanStatus.${attachFile.vc_status}"/>${delYn eq 'Y'?'(삭제)':''}</c:set>
											<c:set var="blockStepClass" value="block_step"/>
											<c:if test="${14 == attachFile.vc_status}">
												<c:set var="blockStepClass" value="none"/>
											</c:if>
										</c:if>
										<c:if test="${(attachFile.dlp_status eq 1 || attachFile.dlp_status eq 2 || attachFile.dlp_status eq 3) && attachFile.send_result eq '500'}">
											<c:set var="inputStyle" value="disabled"/>
											<c:set var="blockStepClass" value="block_step"/>
											<c:set var="delYn" value="Y"/>
										</c:if>
										<c:if test="${(attachFile.apt_status eq 1 || attachFile.apt_status eq 2 
									...
									<c:otherwise>
										<c:set var="fileNameClass">vc_scan_status_info n${attachFile.vc_status eq 4 ? '3' : attachFile.vc_status}</c:set>
										<c:set var="fileVolumeClass">vc_scanStatusText n${attachFile.vc_status eq 4 ? '3' : attachFile.vc_status}</c:set>
										<c:set var="fileVolume"><spring:message code="data.dataView.data.attachFile.deleted"/></c:set>
									</c:otherwise>
									</c:choose>
									<c:set var="isCanDownloadState" value="${delYn eq 'N' && admin_download_yn eq 'Y' && downloadBtnYN eq 'Y' && (data_networkPosition eq 'I' || data.status eq 'SS')}"/>
									<tr <c:if test="${param.ord != null && admin_download_yn eq 'Y' && param.ord == attachFile.ath_ord}">class="hoverColor"</c:if> 
										<c:if test="${param.view eq 'dlp' && attachFile.dlp_status == 1}">class="hoverColor"</c:if>
										<c:if test="${param.view eq 'apt' && attachFile.apt_status == 1 && attachFile.apt_send_yn == 'N'}">class="hoverColor"</c:if>
										>
										
										<td class="first-item c_check">
											<input ${isCanDownloadState ? '' : 'disabled'} type="checkbox" class="input_chk" name="chk" id="chk" value="${attachFile.ath_ord}" />
										</td>
										<td class="c_name name_ellipsis ">
											<span class="title_filename ${fileNameClass}">
												<!-- 업무망은 무조건 파일을 다운로드 받을 수 있다. 인터넷망인 경우에는 전송완료로 넘어와야지만 다운로드할 수 있다. 그러므로 data.status값이 SS일때 다운로드 버튼을 활성시킨다. -->
												<c:if test="${isCanDownloadState}">
													<a id="download${attachFile.ath_ord}" onclick="oneFileDownloadFromLink( '${data_networkPosition}', '${attachFile.data_seq}' , '${attachFile.ath_ord}' , '${fn:replace(attachFile.file_nm, "'", "\\'") }' ); " ><c:out value="${attachFile.file_nm}" /></a>
												</c:if>
												<c:if test="${!isCanDownloadState}">
										...
										<td class="c_size ${fileVolumeClass}">
											...
											<c:if test="${ ( ! isCompressFile || ! isNormalZipFile ) && (attachFile.vc_status eq 11 || attachFile.vc_status eq 12) && (attachFile.file_block_detail_msg ne '암호화된 zip파일은 전송할 수 없습니다. zip 암호화를 해제 한 후 다시 전송해주세요.') && (attachFile.block_step ne 'OLE')}">
										...
										<c:if test="${user_download_yn eq 'Y'}">
		...
		<div class="btn_area_center mg_b15 mg_t5">
			<c:if test="${downloadBtnYN eq 'Y' && admin_download_yn eq 'Y'}">
				<c:if test="${data_networkPosition eq 'I' || data.status eq 'SS'}"> 
				<!-- 업무망은 무조건 파일을 다운로드 받을 수 있다. 인터넷망인 경우에는 전송완료로 넘어와야지만 다운로드할 수 있다. 그러므로 data.status값이 SS일때 다운로드 버튼을 활성시킨다. -->
					<button type="button" class="btn_big theme mg_r5" id="btnDownload" name="btnDownload"
					onclick="downloadFromLink('input[name=chk]:checked','${data.ath_file_cnt}');"><span class="text">&nbsp;다운로드&nbsp;</span><img class="down_loading_img" alt="다운로드로딩바" src="/Images/fileUpload/loading.gif"></button>
				</c:if>
			</c:if>
