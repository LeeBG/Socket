...
function titleFilter(pol_nm, note) {
	var firstField = document.getElementById(pol_nm);
	var secondField = document.getElementById(note);

	firstField.value = firstField.value.replace(/(<([^>]+)>)/ig, "");
	secondField.value = secondField.value.replace(/(<([^>]+)>)/ig, "");
}
...
function resultSessionExpire(response) {
	...
	$(location).attr("href", context + "/sign/index.lin");
}
...
function setsaveUserIdLocalStorage( user_id ){
	var key = getsaveIdKey();
	user_id = aesUtil.encrypt(user_id);
	localStorage.setItem(key, user_id );
}

function getsaveUserIdLocalStorage(){
	var key = getsaveIdKey();
	var user_id = localStorage.getItem( key );
	if (user_id == null || user_id == "") {
		return;
	}
	return aesUtil.decrypt(user_id);
}

function removesaveUserIdLocalStorage(){
	var key = getsaveIdKey();
	localStorage.removeItem(key);
}

function getsaveIdKey(){
	var key = aesUtil.encrypt("saveId");
	return key;
}

function getSubToDate(time) {
	...
	if (flag == 'D') {
		...
		return  time.substr(0, 4) + '월';
	}
}

function fn_btnCopy(){
	var option ="width=850,height=700,toolbar=no,menubar=no,location=no,scrollbars=yes,status=no,pageXOffset=0,pageYOffset=0,resizable=yes";
	window.open("https://case.ftc.go.kr/com/uat/pki/openCrtfctRegist.do", "pkiCopy", option); 
}